...
(deftest decodes-k-quant-superblocks
  (let [scales (concat (repeat 8 1) (repeat 4 0x11))
        q4 (little-buffer 144)
        _ (.putShort q4 (unchecked-short 0x3c00))
        _ (.putShort q4 (unchecked-short 0x3c00))
        _ (doseq [x scales] (.put q4 (unchecked-byte x)))
        _ (dotimes [_ 128] (.put q4 (byte 0)))
        q5 (little-buffer 176)
        _ (.putShort q5 (unchecked-short 0x3c00))
        _ (.putShort q5 (unchecked-short 0x3c00))
        _ (doseq [x scales] (.put q5 (unchecked-byte x)))
        _ (dotimes [_ 32] (.put q5 (byte 0)))
        _ (dotimes [_ 128] (.put q5 (byte 0)))
        q6 (little-buffer 210)
        _ (dotimes [_ 128] (.put q6 (byte 0)))
        _ (dotimes [_ 64] (.put q6 (unchecked-byte 0xaa)))
        _ (dotimes [_ 16] (.put q6 (byte 1)))
        _ (.putShort q6 (unchecked-short 0x3c00))]
    (.flip q4) (.flip q5) (.flip q6)
    (is (= (vec (repeat 256 -1.0))
           (vec (gguf/decode-f32 :q4-k q4 256))))
    (is (= (vec (repeat 256 -1.0))
